...
export default function ToolsHub() {
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
      ...
      {allTools.map(({ category, tools }) => (<div key={category} className="mb-10"><h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2"><span className="w-1 h-5 bg-[#1d9bf0] rounded-full" />{category}</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">{tools.map(({ icon: Icon, label, desc, to, color }) => (<Link key={label} to={to} className="card-hover glass rounded-xl p-4 group flex items-start gap-3"><div className={`w-8 h-8 shrink-0 bg-gradient-to-br ${color} rounded-lg flex items-center justify-center`}><Icon size={16} className="text-white" /></div><div><h4 className="font-semibold text-sm text-white group-hover:text-[#1d9bf0] transition-colors">{label}</h4><p className="text-xs text-[#71767b] mt-0.5">{desc}</p></div></Link>))}</div></div>))}
